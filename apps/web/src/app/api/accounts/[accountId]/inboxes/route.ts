...
export async function GET(req: Request, { params }: Params) {
  ...
  const rows = await sql`
    SELECT id, name, channel_type as "channelType", widget_color as "widgetColor",
           greeting_message as "greetingMessage", is_enabled as "isEnabled"
    FROM inboxes
...
export async function POST(req: Request, { params }: Params) {
  ...
  const sql = neon(databaseUrl);
  const rows = await sql`
    INSERT INTO inboxes (account_id, name, channel_type, greeting_message, widget_color)
    VALUES (
      ${accountId}::uuid,
      ${body.name.trim()},
      ${body.channelType ?? 'web_widget'},
      ${body.greetingMessage ?? null},
      ${body.widgetColor ?? '#6366F1'}
    )
